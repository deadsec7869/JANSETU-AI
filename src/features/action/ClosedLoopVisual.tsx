...
export const ClosedLoopVisual: React.FC<ClosedLoopVisualProps> = ({ 
  ...
  return (
    <div className={`p-3 rounded-2xl bg-slate-950/80 border border-slate-800 backdrop-blur-xl shadow-xl ${className}`}>
      
      {/* Header */}
      {!compact && (
        <div className="flex items-center justify-between pb-2 mb-2.5 border-b border-slate-800/80 text-[11px] font-mono">
      ...
      <div className="flex flex-wrap md:flex-nowrap items-center justify-between gap-1.5 sm:gap-2">
            ...
            <React.Fragment key={stage.key}>
              <div
                className={`flex-1 min-w-[90px] p-2 rounded-xl border transition-all text-center flex flex-col items-center justify-center gap-1 ${
                  isActive
                    ? 'bg-gradient-to-b from-cyan-950/90 to-slate-900 border-cyan-400 text-white shadow-glow-cyan scale-[1.03]'
                    : isPassed
              ...
              >
                <div className="flex items-center justify-center">
                  <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-cyan-300 animate-pulse' : isPassed ? 'text-emerald-400' : 'text-slate-500'}`} />
                </div>
              ...
              {idx < stages.length - 1 && (
                <div className="hidden md:flex items-center justify-center text-slate-600 text-xs px-0.5 font-mono">
                  →
                </div>
              )}
            </React.Fragment>
        ...
        <div className="hidden lg:flex items-center gap-1 px-2 text-[10px] font-mono text-cyan-400/80 shrink-0">
          <RotateCcw className="w-3.5 h-3.5 animate-spin text-cyan-400" style={{ animationDuration: '10s' }} />
          <span>LOOP</span>
        </div>
      </div>
