...
export const DemoOverlay: React.FC = () => {
  ...
  return (
    <aside aria-label="Hackathon Demo Controller" className="fixed bottom-4 left-4 right-4 md:left-auto md:right-6 md:w-[540px] z-50 transition-all duration-300">
      <div className="glass-panel border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-2xl backdrop-blur-2xl overflow-hidden bg-white/95 dark:bg-slate-900/95">
        
        {/* Header Ribbon */}
        <div className="px-4 py-2.5 bg-slate-50/80 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <div className="flex items-center justify-center w-6 h-6 rounded-lg bg-blue-50 dark:bg-blue-950/60 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800">
              <Zap className="w-3.5 h-3.5 animate-pulse text-violet-500" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-extrabold text-slate-900 dark:text-white tracking-wide font-sans">
                  JANSETU 3-MIN DEMO
                </span>
                <span className="text-[10px] font-mono px-1.5 py-0.2 rounded bg-blue-50 dark:bg-blue-950/60 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-700">
        ...
        <div className="px-4 py-2 bg-slate-50/50 dark:bg-slate-950/40 border-b border-slate-200 dark:border-slate-800">
          <DemoProgress />
        </div>
          ...
          <div className="p-4 space-y-3.5 text-xs">
            ...
            <div className="space-y-1">
              <div className="flex items-center justify-between">
                <h4 className="font-bold text-slate-900 dark:text-white text-sm flex items-center gap-1.5">
                  <span className="text-blue-600 dark:text-blue-400">Step {currentStageIndex + 1}:</span>
            ...
            <div className="grid grid-cols-3 gap-2 py-1">
              {currentStage.metrics.map((m, i) => (
                <div key={i} className="p-2 rounded-xl bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800 text-center">
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate">{m.label}</p>
                  <p className="text-xs font-mono font-bold text-blue-600 dark:text-blue-400 truncate">{m.value}</p>
                </div>
              ))}
            </div>
